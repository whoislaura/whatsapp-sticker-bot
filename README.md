...
Bot para grupos de WhatsApp que convierte automáticamente en sticker cualquier foto enviada con una palabra clave en el pie de foto.

## Características

- Se conecta a WhatsApp sin necesidad de la API oficial de negocio, usando [Baileys](https://github.com/WhiskeySockets/Baileys).
- Detecta fotos enviadas en grupos y las convierte a formato sticker (WebP) al vuelo.
...
Una vez conectado, envía una foto en el grupo con `#s` en el pie de foto y el bot responde con el sticker generado.
...
|---|---|
| `TRIGGER_WORD` | Palabra que debe llevar el pie de foto para generar el sticker. Déjala como `''` para que **todas** las fotos del grupo se conviertan automáticamente. |
...
Conserva la carpeta `auth_info/` entre despliegues y reinicios; ahí vive la sesión, y borrarla obliga a volver a escanear el QR.
